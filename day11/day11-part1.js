...
async function allTheseMonkeys() {
  ...
  const ansPart1 = inspectedTimes[0] * inspectedTimes[1];
  console.log(ansPart1);
}

allTheseMonkeys();

// get number for each line
const getNumberFromArr = (arr) => {
  let numbers = arr.map((el) => parseInt(el)).filter((el) => el >= 0);
  return numbers;
};

//map data input into object of Monkeys
...
// to perform value operation for mapping for changes of item Worry Level
...
// Throwing of each item from monkey 0 to last monkey
  ...
  for (let i = 0; i < ROUND; i++) {
    ...
    for (let k = 0; k < numberofMonkeys; k++) {
      ...
      current.startingItems = current.startingItems
        .map((el) => {
          return Math.floor(
            operation(el, current.symbol, current.operationNumber) / 3
          );
        })
        .reverse();
